...
async function main() {
  ...
  if (cmd === 'ingest') {
    ...
    const rows = await exportableContacts(Number(process.argv[3]) || 1000);
    ...
    await suppress(process.argv[3], process.argv[4] || 'opt_out');
    console.log(`Suppressed ${process.argv[3]}.`);
  } else if (cmd === 'erase') {
    console.log(await eraseValue(process.argv[3]));
  } else {
...
async function printStats() {
  const counts = {
    sources: (await get('SELECT COUNT(*) n FROM sources')).n,
    ...
    raw: (await get('SELECT COUNT(*) n FROM raw_records')).n,
    rejected: (await get(`SELECT COUNT(*) n FROM raw_records WHERE status = 'rejected'`)).n,
  };
  console.log('Database stats:', JSON.stringify(counts, null, 2));
  const byStatus = await all('SELECT status, COUNT(*) n FROM contact_points GROUP BY status ORDER BY n DESC');
  console.log('Contacts by status:', byStatus.map((r) => `${r.status}=${r.n}`).join(', '));
  const bySource = await all(`SELECT s.name, s.type, s.resale_allowed, COUNT(cp.id) n
