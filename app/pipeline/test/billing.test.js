...
test('admin/demo customer is seeded with the app key', async () => {
  const c = await getCustomerByKey('admin-key');
  assert.ok(c);
  assert.equal(c.is_admin, 1);
  assert.equal(remaining(c), PLANS.pro.credits);
});

test('charging deducts credits and writes a usage event', async () => {
  const c = await getCustomerByKey('admin-key');
  const before = remaining(c);
  const bill = await charge(c, 'people/search', 5, { delivered: 5 });
  assert.equal(bill.ok, true);
  assert.equal(bill.charged, 5);
  assert.equal(bill.remaining, before - 5);
  const sum = await usageSummary(await getCustomerByKey('admin-key'));
  assert.equal(sum.creditsUsed, 5);
  assert.ok(sum.recent.length >= 1);
});

test('preview/zero-cost charge does not deduct', async () => {
  const c = await getCustomerByKey('admin-key');
  const before = remaining(c);
  const bill = await charge(c, 'people/search', 0, {});
  assert.equal(bill.ok, true);
  assert.equal(bill.remaining, before);
});

test('new free customer gets its own key and 100 credits', async () => {
  const cust = await createCustomer({ name: 'Acme', plan: 'free' });
  assert.match(cust.api_key, /^fe_cust_/);
  assert.equal(cust.credits_included, PLANS.free.credits);
  assert.equal(cust.is_admin, 0);
});

test('charge beyond balance is refused without deducting', async () => {
  const cust = await createCustomer({ name: 'Small', plan: 'free' });
  const bill = await charge(cust, 'export.csv', PLANS.free.credits + 50, { rows: 150 });
